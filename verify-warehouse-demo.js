...
function runWarehouseDemo() {
  ...
  console.log('缓存命中性能:', readTime1 > readTime2 ? 
    `✅ 提升 ${((readTime1 - readTime2) / readTime1 * 100).toFixed(1)}%` : 
    '⚠️ 无明显提升')
  ...
  const testResults = {
    ...
    '数据完整性': retrievedData1 && JSON.stringify(retrievedData1.sensor_data_json_001) === JSON.stringify(sensorData) ? '✅ 数据完全一致，无损存储' : '❌ 数据不一致',
    '缓存性能': readTime1 > readTime2 ? `✅ 第二次读取提升${((readTime1 - readTime2) / readTime1 * 100).toFixed(1)}%` : '⚠️ 性能提升不明显',
    '数据隔离': combinedData && Object.keys(combinedData).length === 2 ? '✅ 多数据源正确隔离 (JSON + HTTP)' : '❌ 数据隔离机制异常',
