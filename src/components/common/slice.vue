<template>
	<div ref="container" class="slice-wrapper">
		<header ref="header" class="card-header" :class="{ center: item.chart_type === 'x_number' }">
        ...
        item.title
			...
			<div class="right-buttons">
				...
				<LegendBtn class="x-btn" v-if="canLegendType.indexOf(item.chart_type) !== -1" :legend.sync="legend" />
				<SyncBtn class="x-btn" @sync="getData" :white="item.chart_type === 'x_number'" />
				<ExpandBtn class="x-btn" @expand="handleFocus" :expand-target="item.i"
					:white="item.chart_type === 'x_number'" />
			</div>
		</header>

		<component :loading="loading" :is="item.chart_type" :id="item.slice_id" :legend="legend" :api-data="apiData" :title="item.title"
			:fields="item.fields" :colorStart="colorStart" :colorEnd="colorEnd" @init="chartInit" @send="sendMessage" />
					...
					<v-card-text>
						<v-container>
							<v-row>
								...
								<v-col cols="8">
									<v-text-field v-model="interval" label="interval" type="number" min="0">
									</v-text-field>
								</v-col>
							</v-row>
						</v-container>
