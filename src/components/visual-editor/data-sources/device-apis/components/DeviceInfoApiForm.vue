<template>
  <div class="device-info-api-form">
    <BaseApiForm
      ref="baseFormRef"
      v-model="formData"
      :rules="formRules"
...
import { deviceDetail, getDeviceConnectInfo, deviceAlarmStatus, deviceAlarmHistory } from '@/service/api/device'
...
const selectedApiType = ref<ApiType>(props.apiType || 'device_detail')
...
const apiTypeOptions = [
  { label: '设备详情', value: 'device_detail' },
  { label: '连接信息', value: 'device_connect' },
  { label: '告警状态', value: 'device_alarm_status' },
  { label: '告警历史', value: 'device_alarm_history' }
]
...
const callApi = async (data: Record<string, any>) => {
  ...
  switch (selectedApiType.value) {
    case 'device_detail':
      return await deviceDetail(deviceId)

    case 'device_connect':
      return await getDeviceConnectInfo({ device_id: deviceId })

    case 'device_alarm_status':
      return await deviceAlarmStatus({ device_id: deviceId })

    case 'device_alarm_history':
      return await deviceAlarmHistory({ device_id: deviceId })
...
const handleSubmit = async (data: Record<string, any>) => {
  try {
    ...
    console.log('设备信息API调用成功:', formattedData)
  } catch (error) {
    console.error('设备信息API调用失败:', error)
