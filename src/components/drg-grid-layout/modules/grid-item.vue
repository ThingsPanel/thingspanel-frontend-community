...
function handleResize(event: MouseEvent) {
  ...
  if ((type === 'resizestart' && state.isResizing) || (type !== 'resizestart' && !state.isResizing)) {
    return;
  }
    ...
    case 'dragmove': {
      ...
      if (state.bounded) {
        const bottomBoundary =
          target.offsetParent.clientHeight - calcGridItemWHPx(props.h, state.rowHeight, state.margin[1]);
...
function calcPosition(x: number, y: number, w: number, h: number) {
  ...
  if (renderRtl.value) {
    out = {
      ...
      width: w === Number.POSITIVE_INFINITY ? w : Math.round(colWidth * w + Math.max(0, w - 1) * state.margin[0]),
      height:
        h === Number.POSITIVE_INFINITY ? h : Math.round(state.rowHeight * h + Math.max(0, h - 1) * state.margin[1])
    };
  } else {
    out = {
