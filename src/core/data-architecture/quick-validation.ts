...
export async function quickValidation() {
  ...
  try {
    const httpResult = await visualEditorBridge.updateComponentExecutor('validation-http-test', 'dual-data-display', {
      dataSource1: {
        type: 'http',
        enabled: true,
        config: {
...
if (typeof window !== 'undefined') {
  ;(window as any).quickValidation = quickValidation(window as any).validateArchitectureDifference =
    validateArchitectureDifference
