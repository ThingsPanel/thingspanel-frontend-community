...
export function useDataSourceState(
  ...
  const localModelValue = ref<ModelValue>(JSON.parse(JSON.stringify(props.modelValue || {
    activeDataSourceKey: '',
    dataSourceBindings: {}
  })));
  ...
  watch(localModelValue, (newLocalValue) => {
  ...
  watch(normalizedDataSources, (sources) => {
  ...
  }, { immediate: true, deep: true }); // immediate: 确保初始化时立即执行; deep: 侦听数据源定义的变化
