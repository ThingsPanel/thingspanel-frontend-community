<template>
  <div class="rounded card p-4">
    ...
    <el-row type="flex" :gutter="10" class="pt-3 pb-4 px-3 el-dark-input">
      <el-col :span="10">
        <el-input :placeholder="$t('SYSTEM_LOG.DEVICE_LOG.PLACEHOLDER18')" size="medium" v-model="params.business_name"
          clearable @clear="handleSearch()">
          <!--        <template slot="prepend">IP</template>-->
        </el-input>
      </el-col>
      <el-col :span="10">
        <el-input :placeholder="$t('SYSTEM_LOG.DEVICE_LOG.ENTERASSETNAME')" size="medium" v-model="params.asset_name"
          clearable @clear="handleSearch()">
      ...
      <el-col :span="6">
        <el-select v-model="params.send_result" :placeholder="$t('SYSTEM_LOG.DEVICE_LOG.SELECTSENDRESULTS')" clearable
          size="medium">
    ...
    <el-table :data="tableData" v-loading="loading">
      ...
      <el-table-column :label="$t('SYSTEM_LOG.DEVICE_LOG.BUSINESSNAME1')" width="150" align="left" prop="business_name">
        <template v-slot="scope">
      ...
      <el-table-column :label="$t('SYSTEM_LOG.DEVICE_LOG.DWVICEGROUPNAME1')" prop="asset_name" width="150">
        <template v-slot="scope">
          <span class="cursor-pointer" @click="handleSearch({ asset_name: scope.row.asset_name })">{{ scope.row.asset_name
          }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('SYSTEM_LOG.DEVICE_LOG.TRIGGERINGTIME1')" prop="cteate_time" width="90"></el-table-column>
      <!-- <el-table-column :label='$t("COMMON.BUSINESS")' ></el-table-column> -->
      <!-- <div class="text-title">{{ $t("COMMON.BUSINESS") }}：</div> -->
      <el-table-column :label="$t('SYSTEM_LOG.DEVICE_LOG.SENDTHERESULIT1')" prop="send_result" align="center" width="80">
        <template v-slot="scope">
