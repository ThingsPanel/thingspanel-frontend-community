  ...
  <div>
    <el-form-item style="width:100%" :label="$t('AUTOMATION.THEN')">
      <div style="display: flex;margin-bottom: 10px" v-for="(action, index) in actions" :key="index">
        ...
        <el-select :no-data-text="$t('COMMON.SELECT_NO_DATA')" ref="actionTypeRef" style="width: 130px;margin-right:20px"
          :placeholder="$t('AUTOMATION.PLACEHOLDER.ACTION_TYPE')" v-model="action.type"
        ...
        <!-- 操作设备 -->
        <CommandDevice ref="commandRef" style="width:100%" v-if="action.type == 'device'" :data="action.data"
          @change="v => handleCommandChange(action, v)" />


        <!-- 激活场景 -->
        <SceneSelector ref="sceneRef" style="width:100%" v-if="action.type == 'scene'" :value="action.value"
          @change="v => handleSceneChange(action, v)" />

        <!-- 告警通知 -->
        <AlarmNotification ref="alarmRef" style="width:100%" v-if="action.type == 'alarm'" :data="action.data"
...
import CommandDevice from "../components/action/CommandDevice";
import SceneSelector from "../components/action/SceneSelector";
import AlarmNotification from "../components/action/AlarmNotification";
    ...
    data: {
      handler(newValue) {
        if (newValue) {
          ...
          console.log("ActionForm.data", JSON.stringify(this.actions))
