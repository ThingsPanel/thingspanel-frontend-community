        ...
        >
            <template v-slot:top>
                <v-toolbar flat color="" class="no-bg text-white">
                    <v-toolbar-title class="font-size-h3 font-weight-bolder">{{ $t("COMMON.CONTROLSTRATRGYLIST") }}</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-dialog v-model="dialog" max-width="1200px" scrollable>
                        <template v-slot:activator="{ on, attrs }">
                            <router-link :to="{ name: 'strategylist'}"><v-btn
                                    color="primary"
                                    dark
                                    class="mb-2 ml-2"
                            >{{ $t("COMMON.RETURN") }}</v-btn></router-link>
                            <v-btn
                                    color="primary"
                                    dark
                                    class="mb-2"
                        ...
                        >
                            <v-card class="card">
                                <v-card-title>
                                    <h5 class="headline text-white">{{ $t("COMMON.ADDSTRATRGYLIST") }}</h5>
                                </v-card-title>
                                <v-card-text>
                                    <v-container>
                                        <v-text-field
                                        ...
                                        <div class="box">
                                            <v-row v-for="(i,index) in tjarr" v-show="editedItem.isshowsb">
                                                ...
                                                <v-col cols="12" xs="12" md="2" class="col-py-0">
                                                    <treeselect v-model="i.asset_id" :multiple="false" :placeholder="$t('COMMON.PLACEHOLDER8')" :clearable="false" :searchable="false" :options="i.device" :normalizer="normalizer" class="treeselect strtreesel width-100" @input="changeStr(index,i.asset_id,1)" />
                                                </v-col>
                                                ...
                                                <v-col cols="12" xs="6" md="1" class="col-py-0" v-show="i.isvalshow">
                                                    <v-select
                                                            :items="symbols"
                                                ...
                                                <v-col cols="12" sm="6" md="1" class="col-py-0" v-show="i.isvalshow">
                                                    <v-text-field
                                                ...
                                                <v-col cols="12" sm="6" md="2" class="col-py-0 text-center" v-show="i.isswitchshow">
                                                    <v-radio-group v-model="i.kgvalue" row  required class="vselect mar-top-8">
                                                        <v-radio
                                                ...
                                                <v-col cols="12" xs="6" md="1" class="col-py-0" v-show="i.isswitchshow"><div class="text-title mar-top-8 text-center vselect">{{ $t("COMMON.CONTINUED") }}</div></v-col>
                                                <v-col cols="12" sm="6" md="1" class="col-py-0" v-show="i.isswitchshow">
                                                    <v-text-field
                                                            v-model="i.duration"
                                                            class="vselect"
                                                    ></v-text-field>
                                                </v-col>
                                                <v-col cols="12" xs="6" md="1" class="col-py-0" v-show="i.isswitchshow"><div class="text-title mar-top-8 text-center vselect">{{ $t("COMMON.MINUTE") }}</div></v-col>
                                                <v-col cols="12" xs="6" md="1">
                                                    <span class="text-white">{{i.unit}}</span>
                                                </v-col>
                                                <v-col cols="12" xs="6" md="1">
                                                    <v-btn :color="i.btncolor" class="tjbtn vselect" small @click="clickData(index,i.event)" >{{i.btn}}</v-btn>
                                                </v-col>
                                            </v-row>
                                            <v-row v-show="editedItem.isshowsj" v-for="(rule,r) in rulesarr">
                                                ...
                                                <v-col cols="12" xs="6" md="3" class="col-py-0">
                                                    <date-picker :type="rule.datetype" class="datepickers strtimes" :placeholder="$t('COMMON.PLACEHOLDER13')" v-model="rule.time" locale="zh-cn" :format="rule.format_times" :locale-config="localeConfig"  auto-submit></date-picker>
                                                </v-col>
                                                <v-col cols="12" xs="6" md="2" class="col-py-0">
                                                    <v-btn :color="rule.btncolor" class="rulebtn mt-5" small @click="addRules(r,rule.event)" >{{rule.btn}}</v-btn>
                                                </v-col>
                                            </v-row>
                                        </div>
                                        <div class="text-title font-weight-bolder my-2">{{ $t("COMMON.CONTROLINSTRUCTION") }}：</div>
                                        <div class="box">
                                            <v-row v-for="(i,m) in instructionsarr">
                                                <v-col cols="12" xs="12" md="2" class="col-py-0">
                                                    <treeselect v-model="i.asset_id" :multiple="false" :placeholder="$t('COMMON.PLACEHOLDER8')" :clearable="false" :searchable="false" :options="i.device" :normalizer="normalizer" class="treeselect strtreesel width-100" @input="changeStr(m,i.asset_id,2)" />
                                            ...
                                            <v-col cols="12" xs="6" md="2" class="col-py-0">
                                                <div v-show="i.isshowswitch">
                                                    <v-radio-group v-model="i.radiovalue" row  class="vselect mar-top-8">
                                                        <v-radio
                                                                v-for="s in switcharr"
                                                                :key="s.id"
                                                                :label="s.name"
                                            ...
                                            <v-col cols="12" xs="6" md="2">
                                                <v-btn :color="i.btncolor" class="tjbtn vselect" small @click="addInstruct(m,i.event)" >{{i.btn}}</v-btn>
                                            </v-col>
                                        </v-row>
                                        </div>
                                        <v-row>
                                            <v-col col="12" class="col-py-0">
                                                <div class="text-title xs-mt-2">{{ $t("COMMON.POLICYSTATUS") }} ：</div>
                                        ...
                                        <v-row  v-show="editedItem.isshowsb">
                                            <v-col col="12" class="col-py-0">
                                                <v-checkbox
            ...
            <template v-slot:item.actions="{ item }">
                <v-btn color="primary" class="mr-4" small @click="editItem(item.id)">{{ $t("COMMON.EDIT") }}</v-btn>
                <v-btn color="error" class="mr-4"  small @click="deleteItem(item)">{{ $t("COMMON.DELETE") }}</v-btn>
            </template>
        </v-data-table>
        <v-pagination
                class="float-right"
...
<style scoped>
    ...
    .vselect .v-select__selection{color: #333 !important;}
    .timebox{
    ...
    .tjbtn{margin-top: 20px;bottom: 10px !important;}
    .datepickers{
        position: relative;
        top: 15px;
    }
    .box{border: 1px solid #5B92FF;padding: 0 8px;margin: 10px 0;border-radius: 4px;padding-top: 10px;}
    .box .vselect{margin-top: 0 !important;padding-top: 0 !important;}
    .box .v-text-field__details,.box .v-messages{min-height: unset !important;}
    .box .v-input__slot{margin-bottom: 0 !important;}
    .pos-rel-top-15{position: relative;top: 15px;}
    .pos-rel-top-25{position: relative;top: 25px;}
    .remarks{position: relative;bottom: 15px;}
    .mar-top-8{margin-top: 8px !important;}
    .box .mar-top-8{margin-top: 8px !important;}
    ...
    export default {
        data: () => ({
            ...
            headers: [
                ...
                { text: "策略操作",class:'text-white',align: "right", value: "actions", sortable: false }
            ],
            desserts: [],
            editedIndex: -1,
            editedItem: {name:'',describe:'',status:1,operator:'||',sort:'100',type:1,isshowsb:true,isshowsj:false,config:{},issued: null},
            defaultItem: {status:1,operator:'||',sort:'100',type:1,isshowsb:true,isshowsj:false},
            nameRules: [
                v => !!v || '请输入策略名称'
            ],
            describeRules: [
            ...
            frequencies:[],
            property:'',
            switcharr:[{id:0,name:'关'},{id:1,name:'开'}],
            relationship:[{id:'||',name:'或者'},{id:'&&',name:'并且'}],
            timetype:[{id:0,name:'单次'},{id:1,name:'每天'}],
            derelationship:'||',
            operator:'||',
            tjarr:[
                {
            ...
            isshowtitle:true,
            isshowzltitle:true,
            strategy:[{id:1,name:'设备条件类型'},{id:2,name:'时间条件类型'}],
            tab:'false',
            rulesarr:[{interval:'',time:'',btn:'新增一行',event:'add',btncolor:'primary',datetype:'time',format_times:'HH:mm'}]
        }),
        ...
        methods: {
            ...
            ajaxListData(){
                ApiService.post(AUTH.local_url + "/automation/index", {business_id: this.business_id, page: 1, limit: 10})
                    .then(({ data }) => {
                        /*console.log('自动化列表');
                        console.log(data);*/
                        if (data.code == 200) {
            ...
            ajaxdata() {
                ApiService.post(AUTH.local_url + "/automation/property", {business_id: this.business_id})
                    .then(({ data }) => {
                        /*console.log('设备列表');
                        console.log(data);*/
                        if (data.code == 200) {
            ...
            changeStr(i,bid,type){
                // console.log('调取策略条件');
                if(bid !== undefined) {
                    ApiService.post(AUTH.local_url + "/dashboard/device", {asset_id: bid})
                        .then(({data}) => {
                            /*console.log('新接口');
                            console.log(data);*/
                            if (data.code == 200) {
            ...
            changeSb(i,id,type){
                if(id !== undefined) {
                    ApiService.post(AUTH.local_url + "/automation/show", {bid: id})
                        .then(({data}) => {
                            /*console.log('字段值');
                            console.log(data);*/
                            if (data.code == 200) {
                ...
                if(type == 1){
                    for(var m=0;m<this.tjarr[i]['assemblyArr'].length;m++){
                        if(id == this.tjarr[i]['assemblyArr'][m]['key']){
                            if(this.tjarr[i]['assemblyArr'][m]['type']==1 || this.tjarr[i]['assemblyArr'][m]['type']==3){
            ...
            changeStrzl(j,bid){
                // 策略指令
                if(bid !== undefined) {
                    ApiService.post(AUTH.local_url + "/automation/instruct", {bid: bid})
                        .then(({data}) => {
                            if (data.code == 200) {
                                console.log('策略指令');
            ...
            ajaxsymbolsData(){
                ApiService.post(AUTH.local_url + "/automation/symbol")
                    .then(({ data }) => {
                        if (data.code == 200) {
                            /*console.log('范围');
                            console.log(data);*/
            ...
            ajaxfrequenciesData(){
                ApiService.post(AUTH.local_url + "/automation/status")
                    .then(({ data }) => {
                        if (data.code == 200) {
                            console.log('频率');
                            console.log(data.data);
            ...
            editItem(id) {
				let _that = this;
                ApiService.post(AUTH.local_url + "/automation/update", {id: id})
                    .then(({ data }) => {
                        if (data.code == 200) {
                            ...
                            if(_that.editedItem.type==1){
                                ...
                                for(var i=0;i<_that.tjarr.length;i++){
                                    ...
                                    for(var p=0;p<_that.tjarr[i]['assemblyArr'].length;p++){
                                        if(_that.tjarr[i]['field'] == _that.tjarr[i]['assemblyArr'][p]['key']){
                                            if(_that.tjarr[i]['assemblyArr'][p]['type']==1 || _that.tjarr[i]['assemblyArr'][p]['type']==3){
                                                // 输入
                                                ...
                                                _that.tjarr[i]['unit'] = _that.tjarr[i]['assemblyArr'][p]['symbol'];
                                            }else{
                                                // 开关
                            ...
                            if(_that.instructionsarr.length>0) {
                                for (var j = 0; j < _that.instructionsarr.length; j++) {
									...
									if (_that.instructionsarr[j]['switch']) {
										for (let m = 0; m < _that.instructionsarr[j]['switch'].length; m++) {
										    if (_that.instructionsarr[j]['field'] == _that.instructionsarr[j]['switch'][m]['key']) {
										        if (_that.instructionsarr[j]['switch'][m]['type'] == 2) {
										            ...
										            _that.instructionsarr[j]['radiovalue'] = _that.instructionsarr[j]['value'];
										            _that.instructionsarr[j]['inputvalue'] = '';
										        } else {
										            _that.instructionsarr[j]['isshowswitch'] = false;
										            _that.instructionsarr[j]['isshowinput'] = true;
										            _that.instructionsarr[j]['inputvalue'] = _that.instructionsarr[j]['value'];
            ...
            deleteItem(item) {
                ...
                if(con == true){
                    ApiService.post(AUTH.local_url+"/automation/delete",{id:id})
                        .then(({ data }) => {
                            // console.log(data);
                            if (data.code == 200) {
                                this.text = '删除成功！';
            ...
            close() {
                ...
                this.editedItem = {status:1,operator:'||',sort:'100',type:1,isshowsb:true,isshowsj:false,issued: null};
                this.tjarr = [
                    {
                        asset_id:null,
                    ...
                    if(val == true){
                        ...
                        if(this.editedItem.type==1) {
                            ...
                            for (var i = 0; i < this.tjarr.length; i++) {
                                if (this.tjarr[i]['operator'] == '' || this.tjarr[i]['operator'] == undefined) {
                                    // 没有逻辑关系
                            ...
                            for(var j=0;j<this.rulesarr.length;j++){
                                var ojb= {time:this.rulesarr[j]['time'],interval:this.rulesarr[j]['interval']};
                                conditions.push(ojb);
                            }
                        }
                        var applyarr = [];
                        for(var i=0;i<this.instructionsarr.length;i++){
                            if(this.instructionsarr[i]['radiovalue'] ==='' && this.instructionsarr[i]['inputvalue'] ===''){
                                applyarr = [];
                            }else if(this.instructionsarr[i]['radiovalue'] !==''){
                                var ojb= {asset_id:this.instructionsarr[i]['asset_id'],field:this.instructionsarr[i]['field'],device_id:this.instructionsarr[i]['device_id'],value:this.instructionsarr[i]['radiovalue']};
                                applyarr.push(ojb);
                            }else if(this.instructionsarr[i]['inputvalue'] !==''){
                                var ojb= {asset_id:this.instructionsarr[i]['asset_id'],field:this.instructionsarr[i]['field'],device_id:this.instructionsarr[i]['device_id'],value:this.instructionsarr[i]['inputvalue']};
                                applyarr.push(ojb);
                            }
                        }
                        ...
                        if(this.editedItem.issued == null){
                            ...
                            if(config.rules.length > 1 || config.apply.length > 1){
                                alert('仅一个条件，和一个指令，且属于同一设备的情况下，方可允许下发！');
                                return false;
                            }else if(config.rules.length == 1 && config.apply.length == 1){
                                if(config.rules[0]['asset_id'] == config.apply[0]['asset_id'] && config.rules[0]['device_id'] == config.apply[0]['device_id']){
                                    console.log('一致');
                                }else{
