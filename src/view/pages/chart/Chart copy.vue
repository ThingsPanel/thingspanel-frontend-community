    ...
    <div class="v-application">
      ...
      >
        ...
        >
          <v-card class="card">
            <v-card-title>
              <h5 class="headline text-white">{{ $t("COMMON.NEWCHARTUNIT") }}</h5>
            </v-card-title>
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col cols="12" xs="12" md="6">
                    ...
                    <treeselect v-model="asset_id" :multiple="false" :clearable="false" :searchable="false" :options="assarr" placeholder="请选择" :normalizer="normalizer" class="treeselect strtreesel charttree mt-2" @input="changeass(asset_id)" />
                  </v-col>
                  <v-col cols="12" xs="12" md="6">
                    <div class="text-title">
                      {{ $t("COMMON.EQUIPMENT") }}：
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn text @click.stop="paneldialog = !paneldialog" class="canclebtn">{{
                $t("COMMON.CANCEL")
                }}</v-btn>
      ...
      <div class="left-nav" id="left-nav" v-show="isShowLeftNav">
        ...
        <ul class="left-nav-ul">
          <li v-for="(proitem,index) in proitems" v-key="proitem" class="left-nav-li">
            <a harf="javascript:;" @click="choosesel(proitem.id)" :class="(proitem.id==proid?'ative-li':'')">{{proitem.name}}</a>
            <ul v-if="proitem.children" class="pl-1 left-nav-ul">
              <li v-for="item in proitem.children" v-key="item" class="left-nav-li">
                <a harf="javascript:;" @click="choosesel(item.id)" :class="(item.id==proid?'ative-li':'')">{{item.name}}</a>
              </li>
            </ul>
          </li>
        </ul>
      ...
      <div class=" rounded p-4  v-application display-block  v-application--is-ltr">
        <div>
          ...
          <div class="expantdiv text-right inlineblock float-right" v-show="isshowtheme">
            <div class="symbol symbol-40 symbol-light-default mt-2 cursor-pointer mr-2"  @click="addpanel">
            ...
            <!--选择主题按钮-->
            <div class="card-toolbar display-inlineblock bg-primary rounded text-white cursor-pointer mr-2 themebox" v-show="isshowtheme">
              <b-dropdown
                      id="themedrop"
                      size="sm"
                      variant="link"
...
<!--                  <i class="ki ki-bold-more-ver text-white"></i>-->
                ...
                <div class="navi navi-hover" style="width: 150px;">
                  <b-dropdown-text tag="div" class="navi-header font-weight-bold" v-for="(t, index) in chartthemes" @click="changeThemes(index)">
...
<!--                  <i class="ki ki-bold-more-ver text-white"></i>-->
                ...
                <!--begin::Navigation-->
                <div class="navi navi-hover" style="width: 150px;">
                  <b-dropdown-text tag="div" class="navi-header font-weight-bold" v-for="(t, index) in bgthemes" @click="changeBgThemes(index)">
                    <span class="font-size-lg">
                     {{t.name}}
                    </span>
                  </b-dropdown-text>
            ...
            <!--全屏按钮-->
            <div class="symbol symbol-40 symbol-light-default mt-2 cursor-pointer mr-2" @click="getFullCreeen" v-show="isshowfull">
              <span class="symbol-label bg-blue">
                <inline-svg
                        :src="expandicon"
                        class="h-100 align-self-end text-white"
          ...
          <div class="expantdiv text-right inlineblock float-right" v-show="isslectshow" @click="clickSelectBox">
        ...
        <div v-show="showSelect" class="slectbox">
          ...
          <!--<div class="col-md-2 float-left" v-show="isshow">
            ...
            <div class="wid-16 float-left xs-mt-0" v-show="pcisshow">
              <el-select v-model="dateindex" @change="changeFilter" :popper-append-to-body="false" class="width-100">
                <el-option
                        v-for="(b, index) in datefilter"
                        :key="index"
                        :value="b.id"
                        :label="b.name"></el-option>
              </el-select>
            </div>
            <div class="wid-16 float-left" v-show="isshow">
              <date-picker type="datetime" class="datepickers strdate" v-model="strdate" locale="zh-cn" format="YYYY/M/D HH:mm:ss" :locale-config="localeConfig"  auto-submit></date-picker>
            </div>
            <div class="wid-16 float-left" v-show="isshow">
              <date-picker type="datetime" class="datepickers enddate" v-model="enddate" locale="zh-cn" format="YYYY/M/D HH:mm:ss" :locale-config="localeConfig"  auto-submit></date-picker>
            </div>

            <div class="wid-10 float-left col-3" v-show="isshow">
              <v-btn color="primary" @click="inserttime()" class="zdybtn height-40">{{ $t("COMMON.SCREEN") }}</v-btn>
            </div>
      ...
      <dashboard
        ...
        />
    <!--移动端资产模态框-->
    <v-dialog v-model="dialog" max-width="500px" append-to-body="true" class="navdialog">
        <v-card>
          <v-card-title class="position-relative">
            <span class="headline font-size-h3">牲畜管理</span>
            <span class="svg-icon svg-icon-primary svg-icon-md closedialog pointer " @click="Close">
              <inline-svg
                ...
                <ul class="scul_1">
                  <li v-for="items in proitems" class="scli_1">
                    <div class="sctitle_1" @click="choosesel(items.id)">
                      <span class="svg-icon svg-icon-primary svg-icon-md mapicon pointer" @click="Close">
                        <inline-svg
...
export default {
  data() {
    return {
      ...
      isshowfull:true,
      paneldialog:false,
      panelarr:[{business_id:'',assarr:[],widget:[{device_id:null,widget_identifier:'',equarr:[]}]}],
      busarr:[],
      text:'',
      vertical: true,
      snackbar: false,
  ...
  methods: {
      ...
      ApiService.post(AUTH.local_url + "/dashboard/business")
              .then(({data}) => {
                ...
                if (data.code == 200) {
                  ...
                  ApiService.post(AUTH.local_url+"/auth/me")
                    .then(({ data }) => {
                      if(data.code==200){
                        console.log(data);
                        ...
                        if(data.data.is_admin==1){
                          ...
                          if(window.screen.width<768){
                            ...
                            document.getElementById('kt_wrapper').classList.add("pad-lef-0");
                            document.getElementById('left-nav').classList.add("wid-265");
                            document.getElementById('chart-right').classList.add("wid-container");
                            document.getElementsByClassName('container')[0].classList.add('wid-100');
    ...
    getproperty() {
      ApiService.post(AUTH.local_url + "/dashboard/property", {wid: this.busid})
              .then(({data}) => {
                console.log('获取资产');
                console.log(data);
                if (data.code == 200) {
    ...
    gettime() {
      console.log('获取时间');
      ApiService.post(AUTH.local_url + "/dashboard/gettime", {chart_id: this.chart_id})
              .then(({data}) => {
                console.log('获取时间');
                console.log(data);
                if (data.code == 200) {
    ...
    changeThemes(selVal) {
      ...
      document.getElementsByClassName('dropdown-menu')[2].classList.remove("show");
    ...
    changeBgThemes(selVal) {
      console.log(selVal);
      console.log(document.getElementsByClassName('dropdown-menu'));
      document.getElementsByClassName('dropdown-menu')[3].classList.remove("show");
      this.bgindex = selVal;
      this.inserttime();
    },
    ...
    changeFilter(selVal) {
      ...
      if (selVal > 0) {
        ...
        ApiService.post(url, {type: this.dateindex})
                .then(({data}) => {
                  console.log('获取实时筛选');
                  console.log(data);
                  if (data.code == 200) {
    ...
    getFullCreeen() {
      this.n++;
      this.n % 2 == 0 ?
              this.outFullCreeen(document) : this.inFullCreeen(document.getElementById('chartpage'));
      this.n % 2 == 0 ?
              this.expandicon = 'media/svg/icons/General/Expand-arrows.svg' : this.expandicon = 'media/svg/icons/General/Scale.svg';
    },
    // 全屏显示
    inFullCreeen(element) {
      ...
      console.log('打开全屏');
      console.log(el);
      let rfs = el.requestFullScreen || el.webkitRequestFullScreen || el.mozRequestFullScreen || el.msRequestFullScreen || el.fullScreen || el.mozFullScreen || el.webkitIsFullScreen;
    ...
    // 打开面板模态框
    addpanel(){
      ApiService.post(AUTH.local_url + "/dashboard/list",{chart_id:this.chart_id})
        .then(({data}) => {
          console.log('获取面板列表');
          console.log(data);
          if (data.code == 200) {
            if(data.data.length>0){
              ...
              // 新增
              this.isedit = 0;
              this.panelarr=[{business_id:'',assarr:[],widget:[{device_id:null,widget_identifier:'',equarr:[]}]}];
            }
    ...
    changebuss(id){
      console.log(id);
      console.log('资产列表');
      ApiService.post(AUTH.local_url + "/dashboard/property", {wid: id})
        .then(({data}) => {
          console.log('获取资产');
          console.log(data);
          if (data.code == 200) {
    ...
    changeass(id){
      console.log(id);
      if(id !== undefined) {
        ApiService.post(AUTH.local_url + "/dashboard/device", {asset_id: id})
          .then(({data}) => {
            console.log('获取组件内容');
            console.log(data);
            if (data.code == 200) {
    ...
    // 新增业务列表
    addpanellist(i){
      var obj={business_id:'',assarr:[],widget:[{device_id:null,widget_identifier:'',equarr:[]}]};
    ...
    // 删除面板数据
    deletepan(id,type){
      ApiService.post(AUTH.local_url + "/dashboard/delete", {id: id,type:type})
        .then(({data}) => {
          console.log('删除面板');
          console.log(data);
          if (data.code == 200) {
    ...
    // 新增设备列表
    addequlist(index,i){
      var obj = {device_id:null,widget_identifier:'',equarr:[]};
    ...
    submitpanel(){
      ...
      if(this.isedit == 0) {
        // 新增
        ApiService.post(AUTH.local_url + "/dashboard/add", {data: data})
          .then(({data}) => {
            console.log('提交资产');
            console.log(data);
            if (data.code == 200) {
        ...
        // 编辑
        ApiService.post(AUTH.local_url + "/dashboard/edit", {data: data})
          .then(({data}) => {
            console.log('提交资产');
            console.log(data);
            if (data.code == 200) {
    ...
    changeStr(id){
      ApiService.post(AUTH.local_url + "/dashboard/component", {device_id: id})
        .then(({data}) => {
          console.log('获取图表内容');
          console.log(data);
          if (data.code == 200) {
    ...
    onPanel(){
      ...
      ApiService.post(AUTH.local_url + "/dashboard/add", {chart_id: chart_id,asset_id:asset_id,device_id:device_id,widget_identifier:widget_identifier})
        .then(({data}) => {
          console.log('添加图表');
          console.log(data);
          if (data.code == 200) {
...
.chart-item{box-shadow: 0px 6px 29px 3px rgba(27, 64, 122, 0.15) !important;}
.chart_active{border:2px solid #ffffff; border-radius: 4px;padding: 5px; }
.chartborder{border:1px solid #847e7e; border-radius: 4px;padding: 5px;}
.wid-16{width: 16%;padding:0 10px;}
.wid-10{width: 10%;padding:0 10px;}
.buttbox{height: 30px;}
.panelbtn{position: absolute;right: 10px;}
.themebox{
  position: relative;
  bottom: 16px;
  padding: 3px;
}
.table td{padding: 0 0.75rem !important;vertical-align: middle !important;}
.custom-v-dropdown {
  &.dropdown-toggle {
    padding: 0;
    &:hover {
...
.themebox:hover,.themebox .dropdown-toggle{background-color: #5B92FF !important;}
.chartbox{
...
.height-40{height: 39px !important;line-height: 39px !important;}
.treeselect{top:9px;}
.el-select{bottom:-9px;}
.zdybtn{position:relative;bottom:-9px;}
.line{
...
.expantdiv{display: inline-block;float: right;position: relative;}
.btnbox{width: 100%;border-radius: 4px;border: 2px solid #1da1f2;color: #1da1f2 !important;text-align: center;height: 40px;line-height: 40px;font-size: 16px;cursor: pointer;background: #fff !important;}
.btnbox:hover{background: #1da1f2;color: #fff};
.left-nav{
...
@media (max-width: 600px) {
  ...
  .xs-mb-0{padding-bottom: 0 !important;}
  .xs-mt-0{padding-top: 0 !important;}
  .scrow{height: 400px !important;overflow: auto;}
  .closedialog{position: absolute;right: 20px;}
  .closedialog.svg-icon.svg-icon-md svg{width: 25px !important;height: 25px !important;}
  .scul_1,.scul_2{padding: 8px 0px;width: 100%;}
  .scul_2{}
  .scli_1,.scli_2{padding: 8px 0px;border-bottom: 1px solid #f6f6f6;color: #333;font-size: 15px;}
  .scli_2{padding-left: 15px;color:#666;}
  .sctitle_1{}
  .sctitle_2{}
  .mapicon{position: relative;bottom: 1px;}
  .wid-16{width: 100%;margin-bottom: 8px;padding-bottom: 10px;}
  .wid-10{width: 100%;margin-bottom: 8px;padding-bottom: 10px;}
}
</style>
<style>
  ...
  .themebox:hover,.themebox .dropdown-toggle{background-color: #5B92FF !important;}
  .vue-treeselect__control{height:40px !important;line-height:40px;}
  .vue-treeselect__placeholder, .vue-treeselect__single-value{line-height:40px;}
  .zdydatepicker:focus{
  ...
  .wid-100{width: 100% !important;max-width: 100% !important;}
</style>
