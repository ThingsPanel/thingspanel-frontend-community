...
const logger = createLogger('CurveChart')
...
export function useCurveData(props: CurveDataProps, colorGroup: ColorGroup[]) {
  ...
  const getTelemetryData = async (device_id: string, key: string, index: number, metricName: string) => {
    const curveWidth = props.properties?.curveWidth || 1
    ...
    const sampleObj = {
      ...
      lineStyle: {
        width: curveWidth,
        color: colorGroup[index]?.line || '#2563EB'
      },
      itemStyle: {
        color: colorGroup[index]?.top || '#2563EB'
      },
      areaStyle: {
        opacity: 0.3,
        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
          ...
          {
            offset: 1,
            color: colorGroup[index]?.bottom || '#93C5FD'
          }
        ])
      },
      emphasis: {
  ...
  const updateData = (deviceId: string | undefined, metricsId: string | undefined, data: any) => {
    if (params.aggregate_window !== 'no_aggregate') {
      logger.info('Update data: Curve chart is aggregate, return directly')
      return
    }
