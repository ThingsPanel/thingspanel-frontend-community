...
function generateReport() {
  ...
  if (totalUnmappedKeys.size > 0) {
    // 保存未映射的键到文件
    fs.writeFileSync('ultimate-unmapped-keys.json', JSON.stringify(Array.from(totalUnmappedKeys), null, 2), 'utf8');
    console.log('📁 未映射的键已保存到 ultimate-unmapped-keys.json');
  ...
  console.log('  3. 处理剩余未映射的键（可能需要手动添加翻译或直接替换）');
