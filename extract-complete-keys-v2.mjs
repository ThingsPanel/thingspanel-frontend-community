...
function processLocaleDirectory(dirPath, prefix = '') {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      console.log(`📁 处理子目录: ${filePath} (前缀: ${subPrefix})`);
      ...
      console.log(`📄 处理索引文件: ${filePath} (前缀: ${prefix})`);
      const keys = processLocaleFile(filePath, prefix);
      Object.assign(allKeys, keys);
    }
  });
...
function main() {
  ...
  // 处理所有子目录（包括 page, custom）
  const subDirs = ['page', 'custom'];
  
  subDirs.forEach(subDir => {
    const subDirPath = path.join(zhCnPath, subDir);
    if (fs.existsSync(subDirPath)) {
      console.log(`\n📁 处理 ${subDir} 子目录...`);
      const subKeys = processLocaleDirectory(subDirPath, subDir);
      Object.assign(allKeys, subKeys);
    }
  });
  ...
  // 保存完整的键值映射
  fs.writeFileSync('complete-key-value-map-v2.json', JSON.stringify(allKeys, null, 2), 'utf8');
  console.log('✅ 完整键值映射已保存到 complete-key-value-map-v2.json');
  ...
  fs.writeFileSync('complete-key-value-map-v2.mjs', output, 'utf8');
  console.log('✅ JavaScript 格式的映射已保存到 complete-key-value-map-v2.mjs');
  ...
  console.log('\n📊 键前缀分布:');
  Object.entries(prefixCount).sort((a, b) => b[1] - a[1]).forEach(([prefix, count]) => {
