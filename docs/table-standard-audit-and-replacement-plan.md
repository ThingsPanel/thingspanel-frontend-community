...
本轮替换状态（2026-09-20）：**已完成静态展示层覆盖**。按本基线完成 44 个计划内未提交表格文件的视觉统一，覆盖 T03-T44 及 T45 的组件示例/看板小表；设备复合组件 `src/components/data-table-page/index.vue` 沿用本节基线。未修改 API、接口字段、请求参数、分页语义、权限、后端逻辑或业务操作。逐页浏览器视觉验收仍需按报告清单人工完成，数据流治理问题继续保留为后续任务。

用户复查补充（2026-09-20）：发现部分页面只补了局部 CSS，未完整覆盖设备列表的表头/内容背景、容器阴影和静态内嵌 `NTable`；已补齐插件管理、场景管理、告警相关表格、设备分组、3 个 `manage/*` 参考页，以及场景联动和设备告警详情中的静态表。补查后全仓 `NDataTable` 均具备主题或 hover 基线，全仓静态 `NTable` 均有明确包装样式。
...
本轮已完成设备列表视觉样板，并以此基线完成 T03-T44 及 T45 中适用表格的展示层替换；具体文件、页面清单、边界和人工核对方法见 [`docs/table-standard-replacement-report.md`](table-standard-replacement-report.md)。本轮改动仅涉及表格布局样式、列展示属性、稳定行键和官方状态标签表现，没有修改 API、请求参数、分页语义、权限、后端逻辑或业务操作。
...
4. **逐页验收**：每个页面记录文件、表格实例、采用的基准样式、是否存在特殊交互和人工核对结果；本轮已生成核对报告，浏览器人工验收仍是提交后的必要动作。
